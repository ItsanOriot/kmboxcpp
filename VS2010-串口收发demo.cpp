...
{	printf("kmbox串口通信demo\r\n");
	char buff[1024];		        //串口收发数据缓冲区
	_com myserial;				      //声明你需要的串口类
	myserial.open(33,115200);	  //打开串口号COM33 波特率115200
	sprintf(buff,"%s","km.move(10,0)\r\n");//向串口要发送数据(鼠标右移10个单位)
	printf("PC->kmbox:%s",buff);//
	myserial.write(buff);		    //串口发送数据
